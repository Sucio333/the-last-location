...
(function initShader() {
    ...
    // En mobile reducimos la resolución interna del render para evitar
    // que el shader consuma demasiados píxeles por frame.
    const isTouchDevice = window.matchMedia('(pointer: coarse)').matches;
    const dpr = Math.min(window.devicePixelRatio || 1, isTouchDevice ? 1.0 : 1.75);
    const renderScale = isTouchDevice ? 0.65 : 1;
    ...
    float fbm(vec2 p){float v=0.0,a=0.5;int max_iter=u_isMobile==1?3:6;for(int i=0;i<6;i++){if(i>=max_iter)break;v+=a*gnoise(p);p*=2.0;a*=0.5;}return v;}
